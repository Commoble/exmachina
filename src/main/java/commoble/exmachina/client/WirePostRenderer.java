...
public class WirePostRenderer implements BlockEntityRenderer<WirePostBlockEntity>
{
	...
	public void render(WirePostBlockEntity post, float partialTicks, PoseStack matrices, MultiBufferSource buffer, int combinedLightIn, int combinedOverlayIn)
	{
		...
		{
			this.renderConnection(post, world, partialTicks, matrices, vertexBuilder, postVector, WirePostBlockEntity.getConnectionVector(connectionPos), 0F);
		}
			...
			if (stack.getItem() instanceof WireSpoolItem)
			{
				...
				if (nbt != null)
				{
					...
					if (positionOfCurrentPostOfPlayer.equals(postPos))
					{
						...
						this.renderConnection(post, world, partialTicks, matrices, vertexBuilder, vectorOfCurrentPostOfPlayer, renderPlayerVec, eyeHeight);
	...
	private void renderConnection(WirePostBlockEntity post, Level world, float partialTicks,
		PoseStack poseStack, VertexConsumer vertexBuilder, Vec3 startPos, Vec3 endPos, float eyeHeight)
	{
		poseStack.pushPose();
