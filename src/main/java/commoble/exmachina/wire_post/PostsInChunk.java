...
public class PostsInChunk implements ICapabilityProvider, INBTSerializable<CompoundTag>
{
	...
	{
		this.positions = set;
		ExMachina.CHANNEL.send(PacketDistributor.TRACKING_CHUNK.with(this::getChunk), new SyncPostsInChunkPacket(this.chunk.getPos(), set));
	}
	...
	{
		double range = ExMachina.SERVER_CONFIG.maxWirePostConnectionRange().get();
