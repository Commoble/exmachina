...
import com.github.commoble.exmachina.content.wire_post.SlackInterpolator;
...
public class WirePostRenderer extends TileEntityRenderer<WirePostTileEntity>
{
	...
	public static int getColor(World world, BlockPos pos, BlockState state, float partialTicks)
	{
		return 0;
...
//		return (int)MathHelper.lerp(lerpFactor, ColorHandlers.UNLIT_RED, ColorHandlers.LIT_RED) * light / 15;
	...
	public void render(WirePostTileEntity post, float partialTicks, MatrixStack matrices, IRenderTypeBuffer buffer, int combinedLightIn, int combinedOverlayIn)
	{
		...
		int lineColor = getColor(world, postPos, postState, partialTicks);
		for (BlockPos connectionPos : connections)
		{
			BlockState otherState = world.getBlockState(connectionPos);
			int red = Math.min(lineColor, getColor(world, connectionPos, otherState, partialTicks));
		...
		for (Hand hand : Hand.values())
		{
			...
			if (stack.getItem() instanceof WireSpoolItem)
			{
				...
				if (nbt != null)
				{
					...
					{
						Vector3d vectorOfCurrentPostOfPlayer = Vector3d.ofCenter(positionOfCurrentPostOfPlayer);
						...
						// first person
						if ((renderManager.options == null || renderManager.options.getPerspective() == PointOfView.FIRST_PERSON))
						{
							...
							handX = MathHelper.lerp(partialTicks, player.prevPosX, player.getX()) + handVector.x;
							handY = MathHelper.lerp(partialTicks, player.prevPosY, player.getY()) + handVector.y;
							handZ = MathHelper.lerp(partialTicks, player.prevPosZ, player.getZ()) + handVector.z;
						...
						{
							handX = MathHelper.lerp(partialTicks, player.prevPosX, player.getX()) - playerAngleZ * handOffset - playerAngleX * 0.8D;
							handY = -0.2 + player.prevPosY + player.getEyeHeight() + (player.getY() - player.prevPosY) * partialTicks - 0.45D;
							handZ = MathHelper.lerp(partialTicks, player.prevPosZ, player.getZ()) - playerAngleX * handOffset + playerAngleZ * 0.8D;
							eyeHeight = player.isCrouching() ? -0.1875F : 0.0F;
						}
						Vector3d renderPlayerVec = new Vector3d(handX, handY + eyeHeight, handZ);
							
						this.renderConnection(post, world, partialTicks, matrices, vertexBuilder, vectorOfCurrentPostOfPlayer, renderPlayerVec, eyeHeight, lineColor);
		...
		MatrixStack matrices, IVertexBuilder vertexBuilder, Vector3d startPos, Vector3d endPos, float eyeHeight, int red)
	{
		...
		Matrix4f fourMatrix = matrices.peek().getModel();
		...
		if (startY <= endY)
		{
			...
			for (int line = 0; line < lines; line++)
			{
				...
				vertexBuilder.vertex(fourMatrix, (float)firstPoint.getX(), (float)firstPoint.getY(), (float)firstPoint.getZ()).color(red, 0, 0, 255).endVertex();
				vertexBuilder.vertex(fourMatrix, (float)secondPoint.getX(), (float)secondPoint.getY(), (float)secondPoint.getZ()).color(red, 0, 0, 255).endVertex();
			}
...
//			System.out.println(x);
	...
	{
		vertexBuilder.vertex(fourMatrix, x * lerp, y * (yLerp), z * lerp).color(red, 0, 0, 255).endVertex();
	}
