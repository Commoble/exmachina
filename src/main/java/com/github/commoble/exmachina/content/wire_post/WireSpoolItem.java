...
public class WireSpoolItem extends Item
{
			...
			else // existing position stored in stack
			{
				...
				else // we clicked a different post that doesn't have an existing connection to the original post
				{
					...
					if (hit != null)
					{
						...
						{
							ExMachina.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity)player), new WireBreakPacket(WirePostTileEntity.getConnectionVector(lowerPos), WirePostTileEntity.getConnectionVector(upperPos)));
							...
							((ServerPlayerEntity)player).connection.sendPacket(new SPlaySoundEffectPacket(SoundEvents.ENTITY_WANDERING_TRADER_HURT, SoundCategory.BLOCKS, hit.x, hit.y, hit.z, 0.5F, 2F));
						}
					}
					// if post wasn't connected, connect them if they're close enough
					else if (pos.withinDistance(lastPos, ExMachina.INSTANCE.serverConfig.max_wire_post_connection_range.get()))
					{
						stack.removeChildTag(LAST_POST_POS);
	...
	{
		double maxDistance = ExMachina.INSTANCE.serverConfig.max_wire_post_connection_range.get();
		if (holder.getPositionVec().squareDistanceTo(Vector3d.ofCenter(connectionPos)) > maxDistance*maxDistance)
		{
			return true;
		}
		TileEntity te = world.getTileEntity(connectionPos);
		return te == null || te.getType() != TileEntityRegistrar.WIRE_POST.get();
	}
	...
	public static void breakPendingConnection(ItemStack stack, BlockPos connectingPos, Entity holder, World world)
	{
		...
		{
			ExMachina.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity)holder),
				new WireBreakPacket(
