...
public class Circuit
{
	...
	public static final Circuit INVALID_CIRCUIT = new Circuit().invalidate();
	 ...
	 * Should only be called from ExtantCircuits.invalidateCircuitAt
	 */
	@Nonnull
	protected Circuit invalidate()
	{
	...
	public void addCircuitComponent(final BlockContext context, final Node nodeA, final Node nodeB)
	{
		...
		if (elementProperties != null)
		{
			...
			{
				circuitElement = elementContext.createCircuitElement(firstNode, secondNode);
			}
			else
			{
				circuitElement = elementContext.createCircuitElement(secondNode, firstNode);
			}
			this.components.put(context.pos, circuitElement);
		}
//		TileEntity te = world.getTileEntity(pos);
//		if (te != null && te instanceof ICircuitElementHolderTE) //TODO
//		{
...
//		}
	 ...
	 */
	@Nonnull
	public static Circuit buildCircuitFromGround(IWorld world, Node groundNode)
	{
	...
	@Nonnull
	private static Circuit expandCircuitFromNode(IWorld world, Circuit circuit, Node baseNode)
	{
		...
		for (BlockPos componentPos : baseNode.connectedComponents)
		{
			...
			BlockContext componentContext = BlockContext.getContext(componentPos, world);
			BlockState componentState = componentContext.state;
			Block componentBlock = componentState.getBlock();
			ElementProperties elementProperties = ComponentRegistry.ELEMENTS.get(componentBlock);
			if (elementProperties == null)
			{
				throw new NullPointerException("Block " + componentBlock.toString() + " at pos " + componentPos.toString() +
			...
			{
//				// ASSUMPTION: components only have two connections // re-evaluate
...
//			if (componentBlock instanceof IElectricalBlock) TODO
//			{
...
//			}
	...
	private void doAnalysis(IWorld world, Node groundNode)
	{
...
//		HashMap<BlockPos, ResistorElement> resistorMap = new HashMap<BlockPos, ResistorElement>();
		...
		HashMap<BlockPos, CircuitElement> vSourceMap = new HashMap<BlockPos, CircuitElement>();
		HashMap<BlockPos, CircuitElement> resistorMap = new HashMap<BlockPos, CircuitElement>();
		int vCounter = 0;
		int rCounter = 0;
		for (CircuitElement element : this.components.values())
		{
			if (element.context.properties.type == ElementType.VOLTAGE_SOURCE)
			//if (element instanceof VoltageSourceElement)
			{
				vSourceMap.put(element.context.context.pos, element);
				element.identifier = vCounter;
				vCounter++;
			}
			else if (element.context.properties.type == ElementType.RESISTOR)
//			else if (element instanceof ResistorElement)
			{
				resistorMap.put(element.context.context.pos, element);
		...
		if (vCounter == 0) // no sources -- matrix solver may crash, but we don't need to solve because all power will be 0, so just do that
		{
			...
			for (CircuitElement resistor : resistorMap.values())
			{
				resistor.power = 0;
			}
			return;
		...
		CircuitElement[] vSourceArray = new CircuitElement[vCounter];
		CircuitElement[] resistorArray = new CircuitElement[rCounter];
		for (CircuitElement source : vSourceMap.values())
		{
			...
			for (int j=0; j<nonGroundNodeCount; j++)
			{
				...
				{	// diagonals across G where i=j=n are equal to sum(1/R) of all R that touch node n
					...
					for (BlockPos pos : checkNode.connectedComponents)
					{
						...
						{
							CircuitElement r = resistorMap.get(pos);
...
//								matrixDataA[i][j] += 1/resistorMap.get(pos).getNominalResistance();
								matrixDataA[i][j] += 1/resistorMap.get(pos).context.getElementValue();
							}
						}
					}
				}
...
//									matrixDataA[i][j] -= 1/resistorMap.get(posi).getNominalResistance();
									matrixDataA[i][j] -= 1/resistorMap.get(posi).context.getElementValue();
								}
							}
						}
					}
...
//				if (nonGroundNodes[i].equals(vSourceArray[j].getPositiveNode()))
				if (nonGroundNodes[i].equals(vSourceArray[j].nodeA))
				{
					matrixDataA[i][nonGroundNodeCount+j] += 1D;
					matrixDataA[nonGroundNodeCount+j][i] += 1D;
				}
				if (nonGroundNodes[i].equals(vSourceArray[j].nodeB))
				{
					matrixDataA[i][nonGroundNodeCount+j] -= 1D;
					matrixDataA[nonGroundNodeCount+j][i] -= 1D;
				}
		...
		for (int j=0; j < vCounter; j++)
		{
			matrixDataZ[nonGroundNodeCount + j] = vSourceArray[j].context.getElementValue();//.getNominalVoltage();
		}
...
//		for (ResistorElement resistor : resistorArray)
		for (CircuitElement resistor : resistorArray)
		{
			...
			double power = voltageDiff * voltageDiff / resistor.context.getElementValue();//.getNominalResistance();	// P = V^2 / R
			resistor.power = power;
		}
		for (CircuitElement source : vSourceArray)
		{
			...
			double power = source.context.getElementValue() * vectorX.getEntry(nonGroundNodeCount + source.identifier);	// P = V * I
			source.power = power;
		}
	}
	...
	public Set<BlockPos> getAllPositions()
	{
		return this.nodes.stream()
			.flatMap(node -> Stream.concat(node.connectedComponents.stream(), node.wireBlocks.stream()))
	...
	public boolean equals(Object other)
	{
		...
		else if (other instanceof Circuit)
		{
			return this.areValuesEqual((Circuit)other);
		}
		else
		{
			return false;
	...
	private boolean areValuesEqual(Circuit other)
	{
